 ...
 */
function utccloud_form_system_theme_settings_alter(&$form, FormStateInterface &$form_state, $form_id = NULL) {
  ...
  foreach ($social_media_pages as $sm_page => $sm_name) {
    ...
    $form['social']['social_media_pages'][$sm_page]['utccloud' . $sm_page] = array (
      '#type' => 'url',
      '#title'  => t($sm_name),
      '#description' => t('Enter the URL of your ' . $sm_name . ' profile.'),
      '#attributes' => array('placeholder' => 'http://'),
      '#default_value' => theme_get_setting('utccloud' . $sm_page),
    );

    $form['social']['social_media_pages'][$sm_page]['utccloud' . $sm_page . '_link_title'] = array(
      '#type' => 'textfield',
      '#title'  => t('Link title'),
      '#description' => t('Enter the link title eg. Follow us on ... You can leave this field blank if you don\'t want use link title.'),
      '#default_value' => theme_get_setting('utccloud' . $sm_page . '_link_title'),
    );
  }
